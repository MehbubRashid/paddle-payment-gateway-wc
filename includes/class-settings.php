...
class Ppgw_Settings
{
	...
	protected function get_vendor_public_key($vendorId, $vendorApiKey) {
		...
		if (is_wp_error($apiCallResponse)) {
			echo esc_html__( 'Something went wrong. Unable to get API response.', 'paddle-payment-gateway-wc' );
			error_log( __( 'Paddle error. Unable to get API response. Method: ', 'paddle-payment-gateway-wc' ) . __METHOD__ . __( ' Error message: ', 'paddle-payment-gateway-wc' ) . $apiCallResponse->get_error_message());
			exit;
		} else {
			...
			if ($oApiResponse->success === true) {
				...
				echo esc_html__( 'Something went wrong. Make sure that Paddle Vendor Id and Paddle Api Key are correct.', 'paddle-payment-gateway-wc' );
				error_log(__( 'Paddle error. Error response from API. Errors: ', 'paddle-payment-gateway-wc' ) . print_r($oApiResponse->error, true));
				exit;
			}
		}
	}
