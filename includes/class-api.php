...
class Ppgw_API {
     ...
     * @param Ppgw_Settings $settings Plugin settings.
     */
    public static function get_pay_url_for_order($order, $customer, $settings) {
		...
		if (is_wp_error($api_call_response)) {
			...
			wc_add_notice( __( 'Something went wrong getting checkout url. Unable to get API response.', 'paddle-payment-gateway-wc' ), 'error');
			error_log(__( 'Paddle error. Unable to get API response. Method: ', 'paddle-payment-gateway-wc' ) . __METHOD__ . __( ' Error message: ', 'paddle-payment-gateway-wc' ) . $api_call_response->get_error_message());
			return json_encode(array(
				'result' => 'failure',
				'errors' => array(__( 'Something went wrong. Unable to get API response.', 'paddle-payment-gateway-wc' ))
			));
		} else {
			...
			if ($api_response && $api_response->success === true) {
				...
				wc_add_notice(__( 'Something went wrong getting checkout url. Check if gateway is integrated.', 'paddle-payment-gateway-wc' ), 'error');
				if (is_object($api_response)) {
					error_log(__( 'Paddle error. Error response from API. Method: ', 'paddle-payment-gateway-wc' ) . __METHOD__ . __( ' Errors: ', 'paddle-payment-gateway-wc' ) . print_r($api_response->error, true));
				} else {
					error_log(__( 'Paddle error. Error response from API. Method: ', 'paddle-payment-gateway-wc' ) . __METHOD__ . __( ' Response: ', 'paddle-payment-gateway-wc' ) . print_r($api_call_response, true));
				}
				return json_encode(array(
					'result' => 'failure',
					'errors' => array(__( 'Something went wrong. Check if Paddle account is properly integrated.', 'paddle-payment-gateway-wc' ))
				));
			}
		}
	}
	...
	public static function check_webhook_signature() {
		...
		if (empty($vendor_public_key)) {
            error_log(__( 'Paddle error. Unable to verify webhook callback - vendor_public_key is not set.', 'paddle-payment-gateway-wc' ));
			return -1;
		}
